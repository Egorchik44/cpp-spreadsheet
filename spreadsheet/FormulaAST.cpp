...
// бит устанавливается, когда необходимы круглые скобки
    enum PrecedenceRule {
        PR_NONE = 0b00,                // никогда не требуюется
        PR_LEFT = 0b01,                // требуется для левого дочернего элемента
        PR_RIGHT = 0b10,               // требуется для правильного дочернего элемента
        PR_BOTH = PR_LEFT | PR_RIGHT,  // требуется для обоих дочерних элементов
    };

    // PRECEDENCE_RULES[родительский][дочерний] определяет, нужны ли круглые скобки
    // должен быть вставлен между родительским и дочерним элементами с определенными приоритетами;
    // для некоторых узлов правила различны для левого и правого дочерних элементов:
    // (X c Y) p Z против X p (Y c Z)
    //
    // Интересными случаями являются те, в которых удаление скобок привело бы к изменению AST.
    // Это может произойти, когда наши правила приоритета для круглых скобок отличаются от
    // грамматический приоритет операций.
    //
    // Анализ конкретного случая:
    // A + (B + C) - всегда в порядке (ничего с более низким грамматическим приоритетом не могло быть записано справа
    //)
    // (например, если бы у нас было A + (B + C) / D, это не было бы проанализировано каким-либо образом
    // это дало бы нам A + (B + C) в качестве подвыражения, с которым нужно иметь дело)
    // A + (B - C) - всегда в порядке (ничего с более низким грамматическим приоритетом не могло быть записано справа
    //) A - (B + C) - никогда не в порядке A - (B - C) - никогда не в порядке A * (B * C) - всегда в порядке (родитель
    // имеет наивысший грамматический приоритет) A * (B / C) - всегда в порядке (у родителя самый высокий
    // грамматический приоритет) A / (B * C) - никогда не бывает хорошо A / (B / C) - никогда не бывает хорошо
    // -(A + B) - никогда не бывает хорошо
    // -(A - B) - никогда не бывает хорошо
    ...
    // (в настоящее время в таблице мы всегда заключаем ее в круглые скобки)
            ...
            ExprPrecedence GetPrecedence() const override {
                switch (type_) {
                    ...
                    case Divide:
                        return EP_DIV;
                    default:
                        // приходится это делать, потому что в VC++ есть предупреждение об ошибке
                        assert(false);
...
FormulaAST::FormulaAST(std::unique_ptr<ASTImpl::Expr> root_expr, std::forward_list<Position> cells)
    ...
    cells_.sort();  // чтобы избежать сортировки в ячейках, на которые даны ссылки
}
